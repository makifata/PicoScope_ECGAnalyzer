...
namespace MDataplot
{
    ...
    {
        //Datas
        public Int32 Wavecount;                                     //Number of overplotted waves.
        ...
        //Variables unique for one picture box.
        ...
        /*
         * Still not clear this feature should be in this class or not. (Toshi)
         */
        ...
        //Add wave + fitting.
        public void Addwave(SIGNAL[] wave)
        {
            //Add new wave.
            wavelist.Add(wave);
            plotflag.Add(true);
            FittingView(true, true); //always fit when wave is added.
            Wavecount++;
        }
        ...
        //this function can control plot is shown or hidden.
        public void SetPlotFlag(int index, bool flag)
        {
            plotflag[index] = flag;
        }
        ...
        public void UpdateView(Double X1, Double X2)
        {
            ...
            Y_start = testarray.Min()-(testarray.Max() - testarray.Min())*0.2;
            Y_end = testarray.Max() + (testarray.Max() - testarray.Min()) * 0.2;
...
//            return (point.X >= X_start && point.X <= X_end && point.Y >= Y_start && point.Y <= Y_end);
        ...
        public void FittingView(Boolean X, Boolean Y)
        {
            ...
            if (y_array.Count >= 2 && Y)
            {
                Y_start = y_array.Min();
                Y_end = y_array.Max();
                //Y_start -= 0.2 * (Y_end - Y_start) / 2;
                //Y_end += 0.2 * (Y_end - Y_start) / 2;
            }
        }
        ...
        public Bitmap Plot()
        {
            ...
            if (Auto_tick)
            {
                ...
                {
                    //LOG10(X) gives digits of the number.
                    Double Tick_log = Math.Log10((X_end - X_start) / (i + Consts.TICK_X_NUM)); //Pick tick number can divide a range with minimum error.
                    Double temp = Math.Abs(Math.Round(Tick_log,0) - Tick_log);
                    if (i == -2 || err_min > temp)
                    {
                        err_min = temp;
                ...
                Exponent_X = (int)(Math.Floor(Math.Log10(X_end))/3)*3;  
                Digits_after_dp_X = -(int)Math.Round(Math.Log10(Tick_inc_X/Math.Pow(10,Exponent_X)), 0);
                if (Digits_after_dp_X < 0 || Digits_after_dp_X > 15) Digits_after_dp_X = 0;
                ...
                {
                    //LOG10(X) gives digits of the number.
                    Double Tick_log = Math.Log10((Y_end - Y_start) / (i + Consts.TICK_Y_NUM)); //Pick tick number can divide a range with minimum error.
                    Double temp = Math.Abs(Math.Round(Tick_log,0) - Tick_log);
                    if (i == -2 || err_min > temp)
                    {
                        err_min = temp;
                ...
                Exponent_Y = (int)(Math.Floor(Math.Log10(Y_end)) / 3) * 3; 
                Digits_after_dp_Y = -(int)Math.Round(Math.Log10(Tick_inc_Y / Math.Pow(10, Exponent_Y)), 0);
                if (Digits_after_dp_Y < 0 || Digits_after_dp_Y > 15) Digits_after_dp_Y = 0;
            }
            ...
            {
                //int label_pos_X = (int)(i * draw_width / Tick_num_X) + label_pos_X_init;
                ...
                if (label_pos_X >= 0 && label_pos_X <= draw_width)
                {
                    ...
                    //(X_start + Math.Round(Tick_inc_X, Digits_after_dp_X) * i) / Math.Pow(10, Exponent_X));
            ...
            for (i = 0; i <= Tick_num_Y; i++)
            {
                ...
                if (label_pos_Y >= 0 && label_pos_Y <= draw_height)
                {
                    ...
                    RectangleF rect = new RectangleF(Consts.fnt.Height*2, Consts.marginY2 + label_pos_Y - Consts.fnt.Height / 2,
                                                        Consts.marginX1 - Consts.fnt.Height*3, Consts.fnt.Height);    //X,Y,Width,Height
                    g.FillRectangle(Plot_Background, rect);
                    String tempstr = String.Empty;
                    ...
                    String fmtstr = string.Format("{0:0." + tempstr + "}",label_value);
                    //String fmtstr = string.Format("{0:0." + tempstr + "}", (Y_start + Math.Round(Tick_inc_Y, Digits_after_dp_Y) * i) / Math.Pow(10, Exponent_Y));
                    g.DrawString(fmtstr, Consts.fnt, Plot_FontColor, rect, stringFormat);
                }
            }
